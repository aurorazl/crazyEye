...
def valid_host_groups_back(request): #deprecated
    ...
    for h_group in host_groups:
        hosts = models.HostToRemoteUser.objects.filter(host_group_id= h_group.id)
...
class MultiTask(object):
    ...
    def run_cmd(self):
        ...
        exec_hosts = models.HostToRemoteUser.objects.filter(id__in=host_ids)
    ...
    def get_task_result(self,detail=True):
        ...
        task_obj = models.Task.objects.get(id=int(task_id))
        task_detail_obj_list = models.TaskLogDetail.objects.filter(task_id=task_obj.id)
        log_dic['summary']={
            'id':task_obj.id ,
        ...
        if detail:
            for log in task_detail_obj_list:
                log_dic['detail'][log.id] = {
                    'date': log.date ,
                    'bind_host_id':log.host_to_remote_user_id,
                    'host_id': log.host_to_remote_user.host.id,
                    'hostname': log.host_to_remote_user.host.name,
                    'ip_addr': log.host_to_remote_user.host.ip_addr,
                    'username': log.host_to_remote_user.remote_user.username,
                    'system' : log.host_to_remote_user.host.system_type,
                    'event_log': log.result,
                    'result': log.status,
                    'note': log.note
                }
